...
import { MarketDashboard } from '@/components/market-dashboard/market-dashboard';
import { FlowTabContent } from '@/components/tabs/flow-tab-content';
import { Flow } from '@/types/flow';
import { ReactNode, createElement } from 'react';

export interface TabData {
  type: 'flow' | 'settings' | 'market-dashboard';
  title: string;
  flow?: Flow;
...
export class TabService {
  static createTabContent(tabData: TabData): ReactNode {
    switch (tabData.type) {
      ...
      case 'market-dashboard':
        return createElement(MarketDashboard);
  ...
  static restoreTab(savedTab: TabData): TabData & { content: ReactNode } {
    switch (savedTab.type) {
      ...
      case 'market-dashboard':
        return TabService.createMarketDashboardTab();
